...
## 各个模块的通信方式
基于ROS2框架，创建各节点，节点中通过发布/订阅话题（异步、单向、多对多），进行通信
## 一些算法
欧几里得聚类
KD树：二叉树，选择轴划分，确定划分点（中位数）；小于中位数的点放入左子树，大约等于的放入右子树；重复前面几步，直到点集中只剩下一个点。
PNP解算
Kalman滤波&融合
...
graph TD
    ...
    D --> I
    E["相机驱动：采集相机图像，发布 camera_image"]--> F["Calibrate：把图像检测坐标转换到世界/场地坐标系，保存 config/out_matrix.yaml"]
...
### TDT_vision
tdt_vision 模块是一个多组件的 ROS 2 视觉感知系统，旨在从单目相机图像中实时提取、定位和跟踪场地内的机器人目标，并输出稳定的3D状态信息。
#### calibrate
实现相机与世界坐标系的空间对齐（确定相机在世界坐标系的位置）让其他节点可以正确地把像素坐标点投入世界坐标系

    1. 初始化创建ROS2节点 "radar_calibrate_node"，打开可视化窗口
    2. 从config/camera_params.yaml文件读取相机内参与畸变参数。读取 usechessboard 参数以判断采用哪种标定方式。初始化固定的真实坐标点real_points
    3. 标定好点后，使用pnp求real_point的位姿，并把外参写入config/out_matrix.yaml
    4. 通过 publish_tf() 将相机到世界坐标系的变换发布为 TF

#### detect
精确识别出场上所有机器人的装甲板中心点、颜色和数字编号。（基于相机坐标系）

    1. 节点通过ROS2订阅camera_image话题，获取原始的相机图像数据（并转换成cv::Mat格式）。
    2. 使用yolo模型粗略识别车辆，筛选过后对图片做一些处理（裁剪）并储存在car_imgs列表
    3. 使用armor_yolo模型对所有车辆小图进行批量推理，精确识别车辆内部的装甲板。将检测到的像素坐标储存
    4. 遍历所有已检测到装甲板的车辆。根据装甲板的坐标，计算出每个装甲板在原始全图上的精确矩形框。（并裁剪，转换格式classify::Image）
    5. 调用classifier模型对所有装甲板小图进行批量推理，得出数字信息
    ...
    9. 将装甲板的中心坐标、颜色和数字信息写入消息vision_interface::msg::DetectResult 中。
    10. 将检测结果发布。

#### resolve
实现RoboMaster雷达站软件架构中的核心定位和可视化功能，把Detect识别到的目标投影回世界坐标系。

    1. 创建ROS2节点 "radar_resolve_node"。读取场地图像 configa/RM2024.png 用于绘制可视化小地图。并订阅相关节点，进行一系列初始化。
    2. 加载 Calibrate 求得的外参矩阵（out_matrix.yaml）
    3. 读取detectresult，遍历装甲板坐标数组，并将其转化为世界坐标系
    4. 将其绘制到地图上（圆点 + 数字标识）；
    5. 同时添加到点云中（方便与雷达点云叠加），用颜色区分红蓝阵营；
    6. 发布转化后结果
